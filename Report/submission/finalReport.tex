...
word embeddings of the paper's title and abstract. One of the things that makes this
...
are concatenated, which helps with stability during training. On standard citation network
...
combining a standard local message-passing step with a global Transformer that lets every
...
is actually better on harder tasks. OGB provides fixed dataset splits, a standardized
...
forward pass reduces to a sparse matrix multiply, which is why GCN is so fast in practice.
...
GCNConv) that aggregates from immediate neighbors. The global branch is a full Transformer
...
All models were implemented in Python 3.11 using PyTorch 2.x and PyTorch Geometric, and
...
measuring accuracy separately within each of the 40 subject categories. This is important
...
\begin{table}[h]
  ...
  \begin{tabular}{lcccc}
  \toprule
  \textbf{Model} & \textbf{Test Acc.} & \textbf{Val Acc.} & \textbf{Macro Acc.} & \textbf{\# Params} \\
   & \textbf{(\%)} & \textbf{(\%)} & \textbf{(\%)} & \\
  \midrule
  GCN & 72.00 & 72.84 & 48.36 & 110K \\
  GAT & 71.75 & 72.85 & 51.26 & 1.36M \\
  GPS & 68.97 & 70.67 & 47.95 & 2.42M \\
  \bottomrule
  \end{tabular}
  \end{table}
...
\begin{figure*}[t]
\centering
\subfloat[GCN (500 epochs)]{\includegraphics[width=0.32\linewidth]{figures/gcn_training_curves.png}}
\hfill
\subfloat[GAT (500 epochs)]{\includegraphics[width=0.32\linewidth]{figures/gat_training_curves.png}}
\hfill
\subfloat[GPS (453 epochs, early stopped)]{\includegraphics[width=0.32\linewidth]{figures/gps_training_curves.png}}
\caption{Training and validation accuracy over epochs for each model. GCN and GAT converge
to similar validation levels around 72--73\%, while GPS levels off about 2 percentage points
lower. The gap reflects the reduced attention context each cluster has under ClusterLoader.}
\label{fig:curves}
\end{figure*}
...
learning rate several times during GPS training in response to flat validation accuracy,
...
together, these factors suggest that GPS's advantage over GCN is not automatic and depends
...
computational cost. Other interesting directions include applying these models to
